  ...
  return (
    <div style={{ marginTop: 32 }}>
      <div style={{ fontSize: '.7rem', fontWeight: 700, color: t.tx3, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
        Mes devoirs
        {pending.length > 0 && (
          <span style={{ background: acc, color: '#fff', borderRadius: 20, padding: '1px 8px', fontSize: '.62rem' }}>{pending.length}</span>
        )}
      </div>
      {sorted.map(a => {
        ...
        return (
          <div key={a.id} onClick={() => onOpenSurah && onOpenSurah(a.surah_n)}
            style={{ padding: '12px 14px', borderRadius: 12, background: t.s1, border: '1px solid ' + (overdue ? '#e91e6333' : t.b1), marginBottom: 8, cursor: onOpenSurah ? 'pointer' : 'default', opacity: a.completed ? .75 : 1 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
              <div style={{ width: 30, height: 30, borderRadius: '50%', background: a.completed ? t.gr + '22' : acc + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.85rem', flexShrink: 0 }}>
                {a.completed ? '✓' : '📝'}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {a.due_date && (
                  <div style={{ fontSize: '.6rem', color: overdue ? '#e91e63' : t.tx3, marginTop: 2 }}>
                    {overdue ? 'Expiré le ' : 'Échéance : '}{new Date(a.due_date).toLocaleDateString('fr-FR')}
                  </div>
                )}
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <div style={{ fontWeight: 700, fontSize: '.8rem', color: a.completed ? t.gr : acc }}>{a.done}/{a.total}</div>
              </div>
            </div>
            <div style={{ height: 4, borderRadius: 2, background: t.b1, marginTop: 10, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: (a.total > 0 ? a.done / a.total * 100 : 0) + '%', background: a.completed ? t.gr : acc, borderRadius: 2, transition: 'width .4s' }} />
            </div>
          </div>
        );
      })}
